...
protected:
    static inline std::string test_endpoint = "tcp://127.0.0.1:5555";
...
TEST_F(CudaIpcMemoryManagerAPITest, CreateAndGetCUDABufferRequest) {
    ...
    GPUBuffer buffer = api->CreateCUDABufferRequest(1024, gpu_id, 0, 0, true);

    EXPECT_NE(buffer.getDataPtr(), nullptr);
    EXPECT_EQ(buffer.getSize(), 1024);
    EXPECT_EQ(buffer.getCudaDeviceId(), 0); // adjust based on your test GPU id
    EXPECT_EQ(buffer.getAccessId(), 0);
